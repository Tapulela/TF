 ...
 * @author usuario
 */
public class ABMLocalidad extends javax.swing.JFrame implements TransferenciaInstancias {
    ...
    private final String textoAlta = "Dar de alta una localidad";
    private final String textoBaja = "Dar de baja una localidad";
    private final String textoModificacion = "Modificar una localidad";
    ...
    private void initComponents() {
        ...
        jLStaticCampo2.setText("Codigo postal");
        ...
        jLStaticCB1.setText("Seleccione una localidad");
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createSequentialGroup()
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addComponent(cabeceraDeVentana, javax.swing.GroupLayout.DEFAULT_SIZE, 1683, Short.MAX_VALUE)
    ...
    private void jBConcretarAccionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBConcretarAccionActionPerformed
        ...
        try {
            switch ((String)jCBOperacion.getSelectedItem()){
                case "Alta":
                    if (jCB3.getSelectedItem() instanceof Provincia)
                        this.organizacion.registrarLocalidad((Provincia) jCB3.getSelectedItem(), jTFCampo1.getText(), jTFCampo2.getText());
                    else
                        throw new ExcepcionCargaParametros("No se ha seleccionado una Provincia para realizar el alta de la localidad.");
                    break;
                case "Baja":
                    if (jCB1.getSelectedItem() instanceof Localidad)
                        organizacion.darDeBajaUnaLocalidad((Localidad)jCB1.getSelectedItem());
                    else
                        throw new ExcepcionCargaParametros("No se ha seleccionado una localidad para dar de baja.");
                    break;
                case "Modificacion":
                    if (jCB1.getSelectedItem() instanceof Localidad)
                        organizacion.modificarLocalidad(((Localidad)jCB1.getSelectedItem()).getProvinciaAsociada(),(Localidad)jCB1.getSelectedItem(),jTFCampo1.getText(), jTFCampo2.getText(), (String) jCBEstado.getSelectedItem());
                    else
                        throw new ExcepcionCargaParametros("No se ha seleccionado una localidad para modificar.");
                    break;
            }
            ...
            Logger.getLogger(ABMLocalidad.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void jCB1ItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jCB1ItemStateChanged
        if (!(jCB1.getSelectedItem() instanceof Localidad))
            return;
        Localidad unaLocalidad = (Localidad) jCB1.getSelectedItem();
        jTFCampo1.setText(unaLocalidad.getNombre());
        jCBEstado.setSelectedItem(unaLocalidad.getEstado());
        jCB2.setSelectedItem(unaLocalidad.getProvinciaAsociada().getPaisAsociado());
    ...
    private void jCB2ItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jCB2ItemStateChanged
        if (jCB2.getSelectedItem()!=null && !jCB2.getSelectedItem().equals("Seleccionar")){
            this.organizarElementos();
        }
    }//GEN-LAST:event_jCB2ItemStateChanged

    private void jCB3ItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_jCB3ItemStateChanged
        if (jCB3.getSelectedItem()!=null && !jCB3.getSelectedItem().equals("Seleccionar")){
            this.organizarElementos();
        }
    }//GEN-LAST:event_jCB3ItemStateChanged
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ABMLocalidad.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ABMLocalidad().setVisible(true);
            }
        });
    }
    ...
    private void prepararAlta() {
        ...
        if (jCB2.getSelectedItem().equals("Seleccionar")){
            cargarPaisesActivos();
            return;
        }
        Pais unPais = (Pais) jCB2.getSelectedItem();
        
        jCB3.setEnabled(true);
        if (jCB3.getSelectedItem().equals("Seleccionar")){
            cargarProvinciasActivas(unPais);
            return;
        }        
    ...
    private void cargarLocalidadesActivas() {
        jCB1.removeAllItems();
        jCB1.addItem("Seleccionar");
        Iterator localidades = organizacion.getLocalidades().keySet().iterator();
        while (localidades.hasNext()){
            int unId = (int) localidades.next();
            Localidad unaLocalidad = (Localidad) organizacion.getLocalidades().get(unId);
            if (unaLocalidad.seEncuentraActiva())
                jCB1.addItem(unaLocalidad);
        }        
    }
    ...
    private void cargarProvinciasActivas(Pais unPais) {
        jCB3.removeAllItems();
        jCB3.addItem("Seleccionar");
